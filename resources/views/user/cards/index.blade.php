...
@section('content')
    ...
    <style>
        ...
        */
        .credit-card {
            aspect-ratio: 1.586 / 1; /* Standard ID-1 Credit Card Ratio */
            border-radius: 16px;
            padding: 1.5rem;
            position: relative;
            color: white;
        ...
        /* Gradient Variants */
        .credit-card.gradient-1 { background: linear-gradient(135deg, #0f172a 0%, #334155 100%); }
        .credit-card.gradient-2 { background: linear-gradient(135deg, #1e1b4b 0%, #4338ca 100%); }
        .credit-card.gradient-3 { background: linear-gradient(135deg, #111827 0%, #374151 100%); }
        /* Default fallback */
        .credit-card-bg {
            position: absolute;
        ...
        */
        .filter-bar {
            ...
            padding: 1rem 0;
            margin-bottom: 2rem;
            /* width: calc(100% + 3rem); */ /* handled by container */
        }
        ...
        @media (max-width: 991.98px) {
            ...
            aside#sidebar.active, aside#sidebar.show, aside#sidebar.expand {
                left: 0 !important;
            }
            ...
            .market-icon { margin-bottom: 1rem; }
            .market-actions {
                width: 100%;
                flex-direction: row;
                justify-content: space-between;
    ...
    <div class="container-fluid px-0">
            ...
            <div class="row row-cols-1 row-cols-md-2 row-cols-xl-3 g-4 wallet-grid-custom">
                @forelse ($userCards as $index => $card)
                    ...
                    <div class="col" id="wallet-card-{{ $card->id }}">
                        <div class="credit-card {{ $gradClass }}" data-bs-toggle="modal" data-bs-target="#modalCardDetails{{ $card->id }}">
        ...
        <div class="container-fluid px-4 pb-5">
            <div class="d-flex justify-content-between align-items-center mb-4">
                ...
                <span class="badge bg-white border text-dark shadow-sm px-3 py-2 rounded-pill">{{ $cards->total() }} Available</span>
            </div>
            ...
            <div class="card border-0 shadow-sm rounded-4 mb-4">
                <div class="card-body p-3">
                    <form method="GET" action="{{ route('user.cards.index') }}" class="row g-2 align-items-center">
                        <div class="col-md-3">
                            <select name="bankname" class="form-select border-0 bg-light fw-bold" style="font-size: 0.9rem;">
                                <option value="">All Banks</option>
                                @foreach ($bankNames as $bank)
                                    <option value="{{ $bank }}" {{ request('bankname') == $bank ? 'selected' : '' }}>{{ $bank }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-3">
                            <select name="cardtype" class="form-select border-0 bg-light fw-bold" style="font-size: 0.9rem;">
                                <option value="">All Types</option>
                                @foreach ($cardTypes as $type)
                                    <option value="{{ $type }}" {{ request('cardtype') == $type ? 'selected' : '' }}>{{ $type }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-3">
                            ...
                            <div class="market-content">
                                <div class="d-flex align-items-center mb-1">
                                    ...
                                    @if($card->joining_fee == 0)
                                        <span class="badge bg-green-100 text-success border border-success-subtle rounded-pill" style="font-size: 0.65rem;">No Fee</span>
                                    @endif
                                </div>
                                <p class="text-muted small mb-0 fw-medium text-uppercase">{{ $card->bank_name }} • {{ $card->network_type }}</p>
                            </div>
                             ...
                             <div class="row g-3 mb-4">
                                 <div class="col-6">
                                     <div class="bg-light p-3 rounded-3 text-center">
                                         <small class="text-muted d-block text-uppercase fw-bold" style="font-size: 0.65rem;">Joining Fee</small>
                                         <span class="fw-bold text-dark">{{ $card->joining_fee == 0 ? 'Free' : '₹'.number_format($card->joining_fee) }}</span>
                                     </div>
                                 </div>
                                 <div class="col-6">
                                     <div class="bg-light p-3 rounded-3 text-center">
                                         <small class="text-muted d-block text-uppercase fw-bold" style="font-size: 0.65rem;">Annual Fee</small>
                                         <span class="fw-bold text-dark">{{ $card->annual_fee == 0 ? 'Free' : '₹'.number_format($card->annual_fee) }}</span>
                                     </div>
                                 </div>
                             </div>
