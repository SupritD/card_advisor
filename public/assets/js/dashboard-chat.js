document.addEventListener("DOMContentLoaded", () => {
    ...
    (async () => {
        try {
            ...
            if (!data.messages || data.messages.length === 0) {
                ...
                const content = data.session.content;
                ...
                //         },
                //         body: JSON.stringify({
                //             message: content, // Force to string to prevent [object MouseEvent]
                //             session_token: token,
            ...
            <div class="message-bubble text-muted">
                ...
                const payload = {
                    message: content,
                    language: document.getElementById("chatLanguage")?.value || "en",
                    session_token: token // Always include token now
                };
    ...
    async function sendMessage(forceText = null, skipUserRender = false) {
        ...
        const text = forceText || content.trim();
        if (!text) return;
